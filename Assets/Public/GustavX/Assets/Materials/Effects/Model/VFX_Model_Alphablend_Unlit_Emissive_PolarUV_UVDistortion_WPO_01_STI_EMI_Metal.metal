...
vertex VertexOutput GustavX_Model_VFX_Model_Alphablend_Unlit_Emissive_PolarUV_UVDistortion_WPO_01_STI_EMI_vertexMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
...
{
	float3 Local0 = (in_0.xyz * in_1.xyz);
	float Local1 = clamp(CB_MaterialConstants.FloatParameter_MaskIntensity, 0.0f, 1.0f);
	float Local2 = (1.0f - Local1);
	//UVNode {
	//} UVNode
	...
	//UVNode {
	//} UVNode
	float2 Local91 = (in_2 * CB_MaterialConstants.Vector2Parameter_StaticTexture_UVTiling);
	...
	//} Texture2DNode
	float3 Local95 = (Local94 * in_1.xyz);
...
fragment PixelOutput GustavX_Model_VFX_Model_Alphablend_Unlit_Emissive_PolarUV_UVDistortion_WPO_01_STI_EMI_fragmentMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
	constant _PerFrame& PerFrame [[buffer(PER_FRAME_BINDING_POINT)]],
	constant _PerCamera& PerCamera [[buffer(PER_CAMERA_BINDING_POINT)]],
	...
	texture2d<float> Texture2DParameter_StaticTexture)
{
	...
	CalculateMatEmissiveColor(CB_MaterialConstants, PerFrame, In.VertexColor, In.InstanceColor, In.TexCoords0, In.InstanceDynamicParameter, matEmissiveColor, _DefaultWrapSampler, Texture2DParameter_DistortionTexture, Texture2DParameter_MaskTexture, Texture2DParameter_MainTexture, _DefaultClampSampler, Texture2DParameter_GradientMap_DefaultClampSampler, Texture2DParameter_StaticTexture);
	Out.Color0 = float4(matEmissiveColor, matOpacity);
