...
vertex VertexOutput GustavX_Model_VFX_Model_Alphablend_Unlit_Emissive_PolarUV_UVDistortion_WPO_01_ST_VEL_vertexMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
	...
	constant const uint8_t* TransformedVertices_prev [[buffer(TRANSPREV_VERT_BINDING_POINT)]],
	...
	texture2d<float> Texture2DParameter_MainTexture)
{
	...
	float4 transformedQTangent_prev = In.LocalQTangent;
	if(CB_EngineParams.TransformedVerticesOffset_prev > 0)
	{
		//Skinned QTangent
		transformedQTangent_prev = GetPrevSkinnedQTangent(VertexID, CB_EngineParams.TransformedVerticesOffset_prev, TransformedVertices_prev);
	}
	//Compute transformedTangentFrame_prev tangent frame
	float3x3 transformedTangentFrame_prev = GetTangentFrame(transformedQTangent_prev);

	float3 transformedNormal_prev = transformedTangentFrame_prev[2];

	//World space Normal_prev
	float3 worldNormal_prev = (float3x3(CB_EngineParams.WorldOffsetMatrix_prev[0].xyz, CB_EngineParams.WorldOffsetMatrix_prev[1].xyz, CB_EngineParams.WorldOffsetMatrix_prev[2].xyz) * transformedNormal_prev);

	//Normalize World Normal (previous frame)
	float3 worldNormalNormalized_prev = normalize(worldNormal_prev);

	CalculateMatWorldPositionOffset_prev(CB_MaterialConstants, PerFrame, TemporalConstants, In.TexCoords0, CB_MaterialConstants.DynamicParameter, worldNormalNormalized_prev, matWorldPositionOffset_prev, _DefaultWrapSampler, Texture2DParameter_DistortionTexture, Texture2DParameter_MainTexture);
	float3 localPositionStatic_prev = In.Position;
	if(CB_EngineParams.TransformedVerticesOffset_prev > 0)
	{
		//Skinned vertex
		localPositionStatic_prev = GetPrevSkinnedPosition(VertexID, CB_EngineParams.TransformedVerticesOffset_prev, TransformedVertices_prev);
	}
	else
	{
		//No prev data
	...
	//World space offset
	float3 worldOffset_prev = (CB_EngineParams.WorldOffsetMatrix_prev * float4(localPositionStatic_prev, 1.0f)).xyz;
...
fragment PixelOutput GustavX_Model_VFX_Model_Alphablend_Unlit_Emissive_PolarUV_UVDistortion_WPO_01_ST_VEL_fragmentMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
	constant _PerFrame& PerFrame [[buffer(PER_FRAME_BINDING_POINT)]],
	constant _PerCamera& PerCamera [[buffer(PER_CAMERA_BINDING_POINT)]],
