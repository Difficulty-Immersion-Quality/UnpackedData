	...
	texture2d<float> Texture2DParameter_VertexColor_MSK)
{
	...
	float Local2 = (CB_MaterialConstants._WorldExtents.y * 0.02f);
	float Local3 = (Local1 / Local2);
	float Local4 = (TemporalConstants.g_CurrentTime_prev + Local3);
	// TriangleWaveNode {
	//TriangleWave
	float Local5 = abs(((fract((Local4 + 0.5f)) * 2.0f) - 1.0f));
	//Smooth TriangleWave
...
vertex VertexOutput GustavX_Effects_VFX_OverlayMaterial_AlphaBlend_Unlit_Emissive_SceneColor_Fresnel_DepthBlend_WPO_LateAlpha_Spectre_01_ST_VEL_vertexMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
	...
	texture2d<float> Texture2DParameter_VertexColor_MSK)
{
	...
	//Object World Offset (previous frame)
	float3 objectWorldOffset_prev = CB_EngineParams.WorldOffsetMatrix_prev[3].xyz;

	//Object World Position (previous frame)
	float3 objectWorldPosition_prev = (objectWorldOffset_prev + TemporalConstants.g_CameraWorldPos_prev);
	...
	//World Position (previous frame)
	float3 worldPosition_prev = (worldOffset_prev + TemporalConstants.g_CameraWorldPos_prev);

	CalculateMatWorldPositionOffset_prev(CB_MaterialConstants, PerFrame, TemporalConstants, CB_MaterialConstants.DynamicParameter, In.Position.xyz, CB_EngineParams.WorldOffsetMatrix_prev, objectWorldPosition_prev, worldPosition_prev.xyz, In.VertexColor, matWorldPositionOffset_prev, _PointWrapSampler, Texture2DParameter_VertexColor_MSK);
...
fragment PixelOutput GustavX_Effects_VFX_OverlayMaterial_AlphaBlend_Unlit_Emissive_SceneColor_Fresnel_DepthBlend_WPO_LateAlpha_Spectre_01_ST_VEL_fragmentMain(constant _CB_MaterialConstants& CB_MaterialConstants [[buffer(MATERIAL_CONSTANTS_BINDING_POINT)]],
	constant _CB_EngineParams& CB_EngineParams [[buffer(ENGINE_PARAMS_BINDING_POINT)]],
	constant _PerCamera& PerCamera [[buffer(PER_CAMERA_BINDING_POINT)]],
